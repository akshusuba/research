...
\caption{t-SNE visualization of learned node embeddings. The model learns type-aware representations, with genes and phenotypes forming adjacent clusters (reflecting direct edges) and metabolites bridging microbes and genes (reflecting their mediating role in the knowledge graph).}
\label{fig:tsne}
\end{figure}
...
\midrule
Full model & 0.908 & 0.924 \\
$-$ Metabolite nodes & 0.848 & 0.849 \\
$-$ Microbe nodes & 0.943 & 0.949 \\
1 layer & 0.921 & 0.863 \\
3 layers & 0.784 & 0.797 \\
\bottomrule
\end{tabular}
\end{table}

Removing metabolites degrades performance (6\% AUROC drop), confirming they contribute to gene-phenotype prediction via multi-hop message passing. The optimal depth is 2 layers; 3 layers overfit on this small graph.
