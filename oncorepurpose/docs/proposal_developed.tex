...
\begin{center}
  ...
  {\large\bfseries\color{sriblue!80!black} A knowledge-graph and language-model pipeline that prioritizes candidates\\ by mechanistic plausibility and literature evidence, with preliminary results}\\[10pt]
...
\begin{winbox}
\textbf{Preliminary findings (already obtained).} (1) On the link-ranking task, a
tuned tabular model (XGBoost) matches or beats the graph network in every regime,
so we do \emph{not} claim the graph wins there. (2) The graph nevertheless carries
real mechanism a tabular model cannot produce: mechanism paths separate true
indications from random pairs at \textbf{AUROC 0.879}. (3) Those paths agree with
the expert-curated DrugMechDB at \textbf{0.802}. (4) A literature-grounded language
model verifies mechanisms with \textbf{0.857 precision} (vs 0.591 for a keyword
baseline). The graph's value is mechanism and explanation; the language model is
...
% ===========================================================================
\section{Discussion: is the project positive?}

The honest answer is yes, but with a clear distinction. The original framing,
``a graph network beats tabular models for repurposing,'' is \emph{false} here, and
we report it. The positive result is different and validated: the graph encodes
real mechanism that a tabular model cannot express, that mechanism separates true
from random indications (AUROC 0.879) and agrees with a curated database (0.802),
and a language model verifies it with high precision (0.857). The deliverable is a
ranked, evidence-backed shortlist with transparent rationales.

\textbf{Limitations, stated plainly.} The separation partly reflects that drugs
indicated for a cancer already have a known target linked to that cancer in the
graph, so this validates that the graph encodes \emph{known} mechanism
consistently, rather than discovering new biology. Some metrics rest on modest
samples (the verifier precision on seven covered ``supported'' pairs) and partial
coverage (DrugMechDB; the open-access full-text subset). Scaling the verifier run
and broadening full-text coverage are the next steps.
