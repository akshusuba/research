...
\begin{center}
  ...
  {\large\bfseries\color{sriblue!80!black} Knowledge-graph mechanism extraction and literature verification\\ that is more precise than keyword screening and reproduces expert curation at scale}\\[10pt]
...
target-lookup and the degree prior collapse to $\sim 0$, while the joint GNN still
recovers the bridge gene (Recall@10 $0.25 \pm 0.02$, per-seed $0.23$--$0.26$,
MRR $0.21$, 3 seeds, $n\approx186$--$260$) from indirect structure. Crucially, a link-only GNN scored through the \emph{identical}
mechanism head (same architecture and scorer, trained without the auxiliary loss)
also recovers nothing (Recall@10 $0.00$), so the capability is attributable to the
auxiliary objective, not to the architecture or a favorable scorer.
...
($\sim$0.25 R@10) but genuine graph-only signal. The link-only GNN uses the same
head and scorer, differing only in the auxiliary loss. Caveats: DrugMechDB supervision
is drug-level (the drug's MOA genes), not pair-specific; even blinded, the bridge gene
may remain reachable through retained PPI/pathway edges, so this reflects recovery of
\emph{known} mechanism via indirect paths, not de novo discovery; and $n\approx200$
covered held-out pairs is modest, so it is a preliminary signal to scale. A real but
narrow advantage, not a blockbuster.}
\label{tab:mechrec}
\end{table}
