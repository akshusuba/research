...
\textbf{Aim 1 (Candidate generation).} Rank drug-cancer pairs with PrimeKG models
(a heterogeneous graph neural network and a knowledge-graph embedding) and a tuned
XGBoost baseline on the same features, and report honestly where the graph helps.\\
\textbf{Aim 2 (Mechanism extraction).} For top candidates, extract multi-hop
...
Do mechanism paths mean anything? & Do true indications carry stronger mechanism paths than random drug-cancer pairs?\\
\rowcolor{srirow} Are the mechanisms real? & Agreement of extracted paths with curated DrugMechDB mechanisms.\\
Is the evidence verification trustworthy? & Among ``supported'' calls, how many are confirmed by curated mechanisms.\\
\bottomrule
\end{tabular}
