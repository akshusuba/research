...
structure-blind control by $+0.047$. And a counterfactual test shows the mechanism head
is \emph{faithful}: deleting the curated mechanism edge, and only that edge, collapses
the prediction ($p\approx3.6\times10^{-34}$). One orthogonal check against
ClinicalTrials.gov is reported as an honest negative.
\end{problembox}
...
\item Mechanism structure separates true indications from random pairs (AUROC 0.879), survives hard negatives, and matches DrugMechDB (0.802).
\item Blinded mechanism recovery: with the direct edge hidden, only the joint GNN names the bridge gene (R@10 0.25 vs $\sim$0 baselines).
\item \textbf{New, prospective:} on a time-split the GNN ranks future indications at AUROC 0.930, beating a structure-blind control by $+0.047$.
...
\end{tikzpicture}
\caption{The OncoEvidence pipeline: rank candidates, extract a mechanism path, verify it
against the literature, output an evidence-backed shortlist.}
\label{fig:pipeline}
\end{figure}
...
nothing, so the signal is attributable to the auxiliary objective, not the architecture.
...
as registered oncology trials more than matched controls. Top raw-score predictions were
not enriched over random (6.7\% versus 7.4\%, Fisher $p=0.69$). Raw score does track trial
existence above chance (AUROC 0.676 over 810 pairs, $p\approx1.5\times10^{-5}$), but this
is a popularity confound: one broadly-indicated drug (folic acid) accounts for 78\% of the
top set's hits. The specificity-lift ranking that de-confounds popularity shows no
enrichment at all (0.55$\times$ versus random, $p=0.97$). We report this as inconclusive:
the raw scores track human trial activity mostly through popular drugs, not through the
specific novel shortlist. A usable version needs MeSH/ChEBI name normalization and trial
phase/recency, which is future work.
\end{nullbox}
...
proxy; and the orthogonal trial check was a negative. None of the predictions are medical
advice.
...
(Finding 5), \texttt{edge\_faithfulness\_eval.json} (Finding 6), and
\texttt{clinicaltrials\_validation.json} (negative). A six-part Colab notebook track
reproduces the pipeline end to end, with Part 4 fully self-contained. All predictions are
