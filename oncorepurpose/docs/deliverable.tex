...
the graph model ranks \emph{future} oncology indications at AUROC 0.930 (vs sampled
negatives) and beats a structure-blind control by $+0.047$; it is \emph{not} better on
median rank, but it \emph{enriches the top-priority tail} --- trained only on pre-2005
structure it surfaces $11\times$ as many future indications in the top 50 as the control,
including post-cutoff approvals like romidepsin and pralatrexate for cutaneous T-cell
lymphoma and nilotinib for CML. And a counterfactual test shows the mechanism head is \emph{faithful}:
deleting the curated mechanism edge, and only that edge, collapses the prediction
($p\approx3.6\times10^{-34}$). A counterfactual stress test then shows the prediction is
causal (target-edge ablation AUROC 0.878), specific (a wrong target is rejected 92.6\% of
the time), and literature-supported over decoy paths; and DepMap CRISPR / GTEx data
independently corroborate that the bridge genes are real cancer dependencies (AUROC 0.607,
$p=4.6\times10^{-6}$), reframing the work as an evidence-triage platform rather than a
ranker. An orthogonal check against ClinicalTrials.gov is reported as
a fully characterized negative: after removing both drug and cancer popularity, the
interaction signal vanishes (AUROC 0.475).
\end{problembox}

\begin{winbox}
\textbf{Nine findings at a glance.}
...
\item \textbf{New, right reason:} a counterfactual stress test shows the prediction is causal (target-edge ablation AUROC 0.878), specific (wrong-target rejection 92.6\%), and literature-supported over decoys (supported 0.80 vs 0.17).
\item \textbf{New, orthogonal:} DepMap CRISPR / GTEx corroborate that true-MOA bridge genes are real cancer dependencies (AUROC 0.607, $p=4.6\times10^{-6}$) with no graph or text; functional features lift the hardest separation to 0.751.
\item \textbf{New, load-bearing retrieval:} evidence-weighting the graph sharpens the shared-target case from 0.618 to 0.742 and demotes coincidental hub paths. Decision-ready extras: conformal abstention (90.3\% coverage), bootstrap CIs, multi-cutoff temporal trend, 83\% non-textbook novelty, and 60 registered predictions.
\end{enumerate}
\end{winbox}
...
\subsection{The deliverable: the converging-evidence lead candidate}
\begin{sloppypar}
The deployment artifact is deliberately \emph{mechanism-first}, not score-first --- and its
front door is not a long ranked list but the single hypothesis on which the most
\emph{independent} evidence layers agree. Because each layer is computed by a separate
experiment, agreement across them is non-trivial. \texttt{converging\_evidence.py} ranks the
novel shortlist this way; the lead is \textbf{Pimecrolimus $\rightarrow$ metastatic melanoma}
via mTOR, where \emph{six} independent layers converge
(\texttt{results/converging\_evidence.md}):
\begin{enumerate}[leftmargin=1.4em,itemsep=0pt,topsep=2pt]
\item \textbf{Graph mechanism:} a direct-target MOA path
\textit{Pimecrolimus $\to$ MTOR $\leftarrow$ melanoma} (specificity lift $+0.76$).
\item \textbf{Conformal calibration:} the candidate is \emph{accepted} (confidence $0.73$), not abstained.
\item \textbf{Mechanism novelty:} labelled \emph{new\_mechanism} --- pimecrolimus is not an
established oncology drug, so this is hypothesis-generating rather than a rediscovered textbook fact.
\item \textbf{Oncology driver context:} the target sits in the PI3K/AKT/mTOR driver axis of melanoma.
\item \textbf{Literature:} the Europe PMC contradiction scan finds no contradicting evidence.
\item \textbf{Functional genomics (independent of the graph and the literature):} MTOR is a real
CRISPR dependency in $71$ melanoma (Skin) cell lines (mean Chronos $-1.06$, DepMap), and the
specificity-aware ranking down-weights pan-essential genes (e.g.\ PCNA) so the lead reflects
lineage-relevant biology rather than generic essentiality.
\end{enumerate}
mTOR inhibition is a genuine melanoma axis, yet pimecrolimus---a topical calcineurin
inhibitor---is a non-obvious repurposing candidate: exactly the kind of mechanism-grounded,
multiply-corroborated hypothesis the platform is built to surface.

\paragraph{Supplementary: the broader mechanism-grounded shortlist.} Behind the lead,
\texttt{scripts/generate\_report.py} emits a per-cancer shortlist that \emph{keeps only}
candidates for which the graph yields a real MOA path (phenotype/symptom coincidences are
dropped by construction, so the list never contains rationales like
``drug $\to$ fatigue $\leftarrow$ cancer''); Table~\ref{tab:dossier} shows representative rows.
This is raw, hypothesis-generating triage --- it deliberately surfaces non-obvious entries
(e.g.\ repositioned metabolic or ion-transport agents) for a human to filter, and is best read
as a candidate pool feeding the convergence analysis above, not as a vetted recommendation.
When a cancer has no mechanism-backed novel candidate in the pool, we say so rather than pad
the list. The full ranked list lives in \texttt{results/repurposing\_shortlist.md}.
\end{sloppypar}
...
\midrule
\rowcolor{srirow} metastatic melanoma & Pimecrolimus (lead) & Pimecrolimus $\to$ MTOR $\leftarrow$ melanoma & $+0.76$\\
glioblastoma & Tamoxifen & Tamoxifen $\to$ PRKCA $\leftarrow$ glioblastoma & $+0.67$\\
\rowcolor{srirow} glioblastoma & Liothyronine & Liothyronine $\to$ PCNA $\leftarrow$ glioblastoma & $+0.67$\\
...
\bottomrule
\end{tabular}
\caption{Supplementary mechanism-grounded shortlist (excerpt from
\texttt{results/repurposing\_shortlist.md}), with the converging-evidence lead at top. Every
row carries a target/PPI/pathway MOA path; non-mechanistic phenotype bridges are excluded.
Candidates are novel (no existing indication edge) and ranked by disease-specific lift. Raw
triage, hypothesis-generating only.}
\label{tab:dossier}
\end{table}
...
\subsection{Finding 7 (new): the model ranks for the \emph{right reason}}
Finding 6 cut a single edge. A stronger question is whether the prediction is
\emph{causally} tied to the named mechanism, \emph{specific} to the right target, and
\emph{supported} by the literature. We run three counterfactuals
(\texttt{counterfactual\_stress\_test.py}, Table~\ref{tab:counterfactual}):
\textbf{(T1) target-edge ablation} --- deleting the curated drug$\to$target edge drops the
mechanism score far more than a matched random edge of the same drug;
\textbf{(T2) wrong-target substitution} --- swapping the true target for a degree-matched,
disease-unassociated decoy is rejected almost always; and \textbf{(T3) decoy-path $+$
verifier} --- replacing the bridge gene with a decoy and re-running the literature verifier,
true bridges are graded \emph{supported} far more often than decoys. The model is not merely
scoring a pair; it relies on the specific, literature-attested mechanism.

\begin{table}[H]
\centering\small
\begin{tabular}{>{\raggedright\arraybackslash}p{0.40\linewidth} >{\raggedright\arraybackslash}p{0.34\linewidth} c}
\toprule
\rowcolor{sriblue}\color{white}\textbf{Counterfactual test} & \color{white}\textbf{Key statistic} & \color{white}\textbf{Sep.\ AUROC}\\
\midrule
\rowcolor{srirow} T1 target-edge ablation (causal) & contrast $0.168$, faithful $87.2\%$, $p\!\approx\!10^{-40}$ & \textbf{0.878}\\
T2 wrong-target substitution (specific) & rejection $92.6\%$, margin $5.7$ & \textbf{0.923}\\
\rowcolor{srirow} T3 decoy-path $+$ verifier (supported) & supported true $0.80$ vs decoy $0.17$ & ---\\
\bottomrule
\end{tabular}
\caption{Counterfactual mechanism stress test. The prediction is causally tied to the named
MOA edge (T1), specific to the right target (T2), and literature-supported over decoys (T3).
T1/T2 on 337 held-out instances; T3 on 30 verifier pairs (lexical verifier; an LLM judge
would sharpen it). The honest weak axis remains shared-target negatives (AUROC $\approx0.61$).}
\label{tab:counterfactual}
\end{table}

\subsection{Finding 8 (new): independent functional-genomics corroboration}
The evidence so far comes from the graph and the literature. We add a third, independent
axis: do the bridge genes behave like real cancer dependencies in data the model never saw?
Using DepMap CRISPR (Chronos) gene-effect and GTEx tissue expression
(\texttt{validate\_functional\_genomics.py}; LINCS skipped, no API key), the bridge genes of
\emph{true} indications are stronger lineage dependencies than those of shared-target hard
negatives (AUROC $0.607$, Mann--Whitney $p=4.6\times10^{-6}$) and more expressed in the
matched tissue (AUROC $0.618$) --- with \emph{no} reference to PrimeKG or text. Folding these
functional features into a specificity classifier lifts the hardest separation (true vs
shared-target) from the path-only $0.609$ to $0.751$ (Table~\ref{tab:funcgen}). Honest
control: a gradient-boosted model on \emph{structure-only} features already reaches $0.731$,
so most of that jump is model capacity; the functional data add a clean, reproducible
increment ($+0.054$ on an interpretable linear model). The value is \emph{orthogonal
corroboration} that the bridge genes are real cancer dependencies, not a leaderboard number.

\begin{table}[H]
\centering\small
\begin{tabular}{>{\raggedright\arraybackslash}p{0.55\linewidth} c c}
\toprule
\rowcolor{sriblue}\color{white}\textbf{Separating true vs shared-target} & \color{white}\textbf{CV AUROC} & \color{white}\textbf{$\Delta$ vs path-only}\\
\midrule
\rowcolor{srirow} Path-only mechanism score (baseline) & 0.609 & ---\\
Linear $+$ DepMap dependency & 0.663 & $+0.054$\\
\rowcolor{srirow} GBM, structure-only (honest control) & 0.731 & $+0.121$\\
GBM, all features ($+$DepMap$+$GTEx) & \textbf{0.751} & $+0.141$\\
\bottomrule
\end{tabular}
\caption{Hard-negative-aware specificity. DepMap CRISPR dependency and GTEx expression are
independent of the graph and the literature; they corroborate the bridge genes and add a
reproducible increment over a strong structure-only control. 400 true vs 368 shared-target
pairs, 5-fold CV.}
\label{tab:funcgen}
\end{table}

\subsection{Finding 9 (new): evidence weighting makes retrieval load-bearing}
A structure-only score treats a coincidental drug$\to$hub-gene$\to$cancer chain like a real
mechanism. Re-weighting each path by Europe PMC evidence (co-mention strength, recency,
bridge-gene specificity, minus a contradiction penalty;
\texttt{evidence\_weighted\_graph.py}) leaves the already-saturated true-vs-random
separation unchanged ($0.886\!\to\!0.890$) but sharpens the hard case substantially:
true vs shared-target rises $\mathbf{0.618\!\to\!0.742}$. Concretely, a textbook-looking
structural hit such as \emph{Insulin}$\to$RB1$\to$gastrointestinal stromal tumor (structure
score $3.08$) collapses to $0$ because the drug is \emph{never} co-mentioned with that cancer,
and literature hubs (e.g.\ NR3C1, $5.6$M mentions) are demoted in favor of specific bridges.
The retrieval layer is load-bearing exactly where it matters: separating established
mechanisms from biologically similar look-alikes. (Caveat: the decisive term is a
drug$\times$cancer co-mention prior, so it corroborates known biology rather than ranking
genuinely novel candidates.)

\subsection{Decision-readiness and statistical hardening}
Four additions make the triage honest as a tool rather than a demo. \textbf{Calibrated
abstention}: split-conformal calibration yields $90.3\%$ empirical coverage at a $90\%$
target while abstaining on $96\%$ of negatives, so the shortlist can say ``insufficient
evidence'' instead of guessing. \textbf{Statistical hardening}: bootstrap $95\%$ CIs confirm
the headline separations (true-vs-random $0.879$ $[0.856,0.900]$; shared-target $0.609$
$[0.569,0.649]$), and the prospective edge holds across three cutoffs
($T{=}2000/2005/2010$, graph gain $+0.065/+0.037/+0.029$), so it is not a single-cutoff
artifact. \textbf{Novelty triage}: of the mechanism-grounded shortlist, $\sim$$83\%$ are
\emph{not} textbook MOAs (a DrugMechDB-grounded label), rebutting the ``rediscovered obvious
facts'' concern. \textbf{Registered predictions}: a timestamped, commit-hashed set of $60$
novel mechanism-backed predictions is filed for future falsification against trials/approvals.
...
so the explanation reflects the biology the model actually used. Findings 7--9 push past
faithfulness to \emph{evidence quality}: the model ranks for the right reason (causal,
specific, supported), DepMap/GTEx independently corroborate the bridge genes, and an
evidence-weighted retrieval layer sharpens the hardest case --- so the contribution is the
\emph{evaluation}, an evidence-triage platform that tests whether a proposed mechanism
actually matters, not another KG+GNN+LLM ranker.
...
\texttt{edge\_faithfulness\_eval.json} (Finding 6),
\texttt{counterfactual\_stress\_test.json} (Finding 7),
\texttt{functional\_genomics\_validation.json} (Finding 8),
\texttt{evidence\_weighted\_graph.json} (Finding 9), the decision-readiness records
(\texttt{conformal\_triage.json}, \texttt{stats\_hardening.json},
\texttt{mechanism\_novelty.json}, \texttt{registered\_predictions\_20260628.json},
\texttt{converging\_evidence.json} --- the single best multiply-corroborated candidate),
and \texttt{clinicaltrials\_validation.json}
with \texttt{clinicaltrials\_deconfounded.json} (the characterized negative). A ten-part Colab notebook track
reproduces the pipeline end to end (Part 4 / the master notebook is fully self-contained; Parts 7--8 cover
the prospective named cases and the popularity-deconfounded check; Parts 9--10 add the counterfactual
stress test and functional-genomics corroboration). All predictions are
