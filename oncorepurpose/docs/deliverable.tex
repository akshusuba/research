...
independently confirm the bridge genes are real cancer dependencies (AUROC 0.607,
...
\item \textbf{New, orthogonal:} DepMap CRISPR / GTEx confirm true-MOA bridge genes are real cancer dependencies (AUROC 0.607, $p=4.6\times10^{-6}$) with no graph or text; functional features lift the hardest separation to 0.751.
...
\paragraph{The single best converging-evidence candidate.} The point of the platform is that
the layers are \emph{independent}, so when several agree on the same novel hypothesis the
signal is non-trivial. \texttt{converging\_evidence.py} ranks the shortlist by how many
independent layers converge. The lead is \textbf{Pimecrolimus $\rightarrow$ metastatic melanoma}
via mTOR, where \emph{six} independent layers agree
...
\begin{table}[H]
\centering\small
\begin{tabular}{>{\raggedright\arraybackslash}p{0.50\linewidth} c c}
\toprule
...
\texttt{mechanism\_novelty.json}, \texttt{registered\_predictions.json},
