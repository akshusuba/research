...
- [x] **Ten-part Colab learning track** + self-contained master notebook (`oncoevidence_part4_full.ipynb` is the official master; `oncoevidence_colab.ipynb` is the Colab copy). All notebooks ship as **reproducible templates** with outputs cleared; run top-to-bottom on a GPU (`FAST_MODE=False`) to regenerate. The headline numbers each one produces are recorded inline and in `results/*.json`. Parts 9--10 add the counterfactual stress test and functional-genomics corroboration.
